...
Analytic optical transport uses centre-of-pixel rays and explicit intersections against compiled MuJoCo plane and oriented-box geometry. Under `finite_plane_visual_extent_v1`, a controlled plane intersection is accepted only when the local x/y hit lies inside compiled `geom_size[0:2]`; MuJoCo's infinite collision plane is not the optical surface. Each direction records exact `int32` vectors at 1024 units per image pixel plus mandatory `uint8` validity and reason masks. A one-pixel four-neighbour band derived from finite analytic surface assignment excludes fragile source and target boundary samples. Target visibility is checked by tracing from the compiled target camera to the same world point with relative tolerance `1e-7 * max(1.0, distance)` and ray epsilon `1e-12`. RGB, depth, and rendered segmentation do not define the vectors or their identity. Full equations, reason precedence, tolerance, closed-form evidence, and limitations are in `docs/GATE_0B_SLICE_3_ANALYTIC_FLOW.md`.
...
The corrected Slice 3 version matrix advances only changed wire contracts. Both resolved configuration schemas remain `0.1.0-dev.2`. The dataset manifest remains `0.1.0-dev.3` because its episode analytic-identity field and wire shape are unchanged. Transition is `0.1.0-dev.4` because the public v2 intersection/visibility metadata is new. Both scene-specific privileged-instrumentation schemas are `0.1.0-dev.4` because exact-disagreement evidence replaces the fractional threshold contract. Slice 1 used transition `0.1.0-dev.1`; its pinned episode-0 ecological hash was `8f7c7a7e8f70f9e84bf2f256ecf93f8d94f5327abe536c01a6b8f7e87aef3df0`. Slice 2 used `0.1.0-dev.2`; the scientifically rejected PR #5 revision used transition and instrumentation `0.1.0-dev.3`.
