...
- `ECOLOGICAL_ORACLE`: remapped surface regions, segmentation-derived boundary contacts, frame-normalised projected-image fractions, region correspondence, neutral raster mask changes, typed analytic optical transport with validity/reasons, typed ecological-visibility-event availability, and typed occlusion availability/relations.
...
Analytic optical transport uses centre-of-pixel rays and explicit intersections against compiled MuJoCo plane and oriented-box geometry. Each direction records exact `int32` vectors at 1024 units per image pixel plus mandatory `uint8` validity and reason masks. A one-pixel four-neighbour band derived from analytic surface assignment excludes fragile source and target boundary samples. Target visibility is checked by tracing from the compiled target camera to the same world point with a relative distance tolerance of `1e-7 * max(1, distance)`. RGB, depth, and rendered segmentation do not define the vectors or their identity. Full equations, reason precedence, tolerance, closed-form evidence, and limitations are in `docs/GATE_0B_SLICE_3_ANALYTIC_FLOW.md`.

Whole-dataset validation independently recompiles each scene and recomputes both directions, then compares every vector component, validity bit, and reason code. Privileged renderer diagnostics compare analytic assignments with raw renderer segmentation only away from analytic boundaries and require at least 0.90 interior agreement. They are consistency alarms, not inputs to the analytic annotation or identity.
...
The Slice 3 version matrix advances only changed wire contracts. Both resolved configuration schemas remain `0.1.0-dev.2`. Transition and dataset-manifest schemas are `0.1.0-dev.3` because available flow artifacts and the episode analytic identity are new. Both scene-specific privileged-instrumentation schemas are `0.1.0-dev.3` because the serialized analytic/renderer diagnostics are new. Slice 1 used transition `0.1.0-dev.1`; its pinned episode-0 ecological hash was `8f7c7a7e8f70f9e84bf2f256ecf93f8d94f5327abe536c01a6b8f7e87aef3df0`. Slice 2 used `0.1.0-dev.2` after replacing causal-sounding raster events and a bare occlusion tuple with explicit typed semantics.
