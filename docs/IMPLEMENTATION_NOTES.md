...
strict scene-family-discriminated YAML config
  ...
  -> analytic oriented edge-lattice ownership from verified attachments and counterfactual rays
...
Occlusion uses a discriminated availability contract. Single-occluder transitions retain `status: available`, the `counterfactual_occluder_exclusion_v1` rule, and independently checked relations that must agree with oriented ownership. Corridor oriented-boundary recomputation verifies that every controlled-controlled edge is attached or locally multi-surface ambiguous and that no occluding contour exists. Corridor occlusion is therefore `status: available`, `relations: []`, under `oriented_boundary_ownership_v1`. This oracle-supported known-empty result remains distinct from the historical unavailable posture.
