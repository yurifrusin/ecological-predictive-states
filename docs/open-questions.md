...
`PUBLIC_REPOSITORY_ONLY` permits exact-head review through repository content, reproducible
artifacts, CI, and public or connected GitHub records. It does not mean that every repository is
public and does not permit unpublished private off-platform evidence. PR #17 is in the private
`yurifrusin/ecological-predictive-states` repository, so its Actions artifacts use connected authenticated
